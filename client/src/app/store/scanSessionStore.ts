...
import {DtoDispatchSheet, DtoScanSession, DtoScanSessionArticle, Ort, ScanSessionType} from '../api/openapi/backend';
  ...
  withMethods(
    (
      ...
      return {
        ...
        startNewScanSession: rxMethod<{ sessionType: ScanSessionType; ort: Ort; dispatchSheetId: number | null }>(
          pipe(
            tap(() => patchState(store, { isLoading: true, error: null })),
            switchMap(({ sessionType, ort, dispatchSheetId }) => scanSessionsService.createScanSession(sessionType, ort, dispatchSheetId)),
            switchMap(scanSession => scanSessionsService.getScanSessionArticles(scanSession.id!).pipe(map(articles => ({scanSession, articles})))),
            tapResponse({
              next: (r) => setupSessionAndStopLoading(r.scanSession, r.articles),
        ...
        createDispatchSheetAndStartScanSession: rxMethod<string>(
          pipe(
            tap(() => patchState(store, { isLoading: true, error: null })),
            switchMap(name => dispatchSheetsService.addDispatchSheet({name})),
            switchMap(dispatchSheet => scanSessionsService.createScanSession(ScanSessionType.ProcessDispatchList, Ort.Lager, dispatchSheet.id!)),
            switchMap(scanSession => scanSessionsService.getScanSessionArticles(scanSession.id!).pipe(map(articles => ({scanSession, articles})))),
            tapResponse({
              next: (r) => setupSessionAndStopLoading(r.scanSession, r.articles),
              error: (error: Error) => displayErrorAndStopLoading(error, 'Scan-Session konnte nicht gestartet werden')
